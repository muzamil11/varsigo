 ...
 *  it against NEXT_PUBLIC_ADMIN_EMAIL before touching the database — see
 ...
 *  upload alike (no `approved` filter), same as rejectUpload below. */
...
export async function fetchAdminStats(adminEmail: string): Promise<AdminStats> {
  ...
  try {
    const [
      ...
      supabase.from('departments').select('id', { count: 'exact', head: true }).eq('university', 'NED'),
    ...
    return {
      ...
      totalCourses: courses.error && isMissingSchemaError(courses.error) ? 0 : courses.count ?? 0,
