import { callAdminFunction } from '@/lib/adminFunction';
...
export async function fetchPendingLostFoundItems(): Promise<LostFoundItem[]> {
  const rows = await callAdminFunction<RawLostFoundRow[]>('listPendingLostFound');
  return rows.map(mapItem);
}

export async function approveLostFoundItem(itemId: string): Promise<void> {
  await callAdminFunction<void>('approveLostFoundItem', { itemId });
}

export async function rejectLostFoundItem(itemId: string): Promise<void> {
  await callAdminFunction<void>('rejectLostFoundItem', { itemId });
}
