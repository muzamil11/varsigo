...
// this file to match (or generate it with `npx supabase gen types typescript`
// once the Supabase CLI is linked to the project).
//
// Deliberately NOT wired into `createClient<Database>()` — the installed
// @supabase/supabase-js (2.110.x) resolves the schema-name generic through
// an internal `__InternalSupabase: { PostgrestVersion }` marker that only
...
// annotates `.select<...>()` / return values with the Row types below.
